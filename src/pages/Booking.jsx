...
export default function Booking() {
  ...
  const item = useMemo(() => {
    ...
    return type === "tour"
      ? tours.find((t) => t.id === Number(id))
      : hotels.find((h) => h.id === Number(id));
  }, [id, type]);
  ...
  if (!item) return <div style={{ color: "#6b7280" }}>Элемент табылган жок.</div>;

  const title = "title" in item ? item.title : item.name;
  const price = "price" in item ? item.price : item.pricePerNight;
    ...
    <div className="container" style={{ paddingTop: 90, paddingBottom: 24, maxWidth: 800 }}>
      ...
      <form onSubmit={handleSubmit} style={{ display: "grid", gap: 12 }}>
        ...
        <div>
          <button
            type="submit"
            style={{ padding: "10px 16px", borderRadius: 10, background: "#198754", color: "white", border: "none" }}
          >
            Төлөмгө өтүү
          </button>
        </div>
